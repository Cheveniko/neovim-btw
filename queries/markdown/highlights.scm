; Local override for Neovim 0.12.1.
; The bundled markdown query uses `conceal_lines` for fenced-code delimiters,
; which currently crashes the core Tree-sitter highlighter on fenced blocks.
; Keep the same highlights, but avoid that metadata until core is fixed.
